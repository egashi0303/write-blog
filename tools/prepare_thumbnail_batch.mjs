...
const manifest = {
  ...
  entries: entries.map((entry) => ({
    ...
    directImagePromptPath: entry.directImagePromptPath,
...
await fs.writeFile(
  ...
  [
    ...
    `Batch ID: ${batchId}`,
    "",
    "1. Open each `thumbnail_prompt.md` and try Canva generation first.",
    "2. If Canva returns quota limit, monthly AI limit, timeout, or a text-heavy result, stop for that item.",
    "3. Open `direct_image_prompt.md` for that item and generate the same scene directly.",
    "4. Update each `thumbnail_generation_report.json` with the actual result path or URL.",
    "5. Run `tools/publish_thumbnail_reports_batch.mjs` with the report paths.",
  ].join("\n"),
  "utf8",
);
